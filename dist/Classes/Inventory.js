...
class Inventory {
    ...
    addPack(pack) {
        ...
        if (found)
            found.amount += pack.amount ? pack.amount : 1;
        else
    ...
    removePack(pack) {
        ...
        if (found.amount > 1)
            ...
            this.packs.splice(this.packs.findIndex((e) => e.code === pack.code), 1);
        return true;
    }
    ...
    fromJSON(json) {
        ...
        for (let item of inventory.getItems()) {
            ...
            items.push(new Item_1.default(iData));
        }
        Object.assign(inventory.list, items);
        const activeItems = [];
        for (let item of inventory.getActiveItems()) {
            const iData = typeof item === "string" ? JSON.parse(item) : item;
            activeItems.push(new Item_1.default(iData));
        }
